...
function LinearAlgebra.normalize!(psi::AbstractMPS; iscanonical::Bool=false)
...
LinearAlgebra.normalize(psi::AbstractMPS; iscanonical::Bool=false) = normalize!(copy(psi); iscanonical=iscanonical)

function Base.:*(psi::MPS, f::Number)
    T = scalartype(psi)
    r = MPS{T}(copy(raw_data(psi)))
    r[1] *= convert(T, f)
    return r
end 
